...
else: sweep_id = 'yfm1n9wv'

if sweep_id == None:
    # Define the sweep configuration
    sweep_id = wandb.sweep(sweep={
            'method': 'bayes',
            'name': 'Simple Mean + Vegetation Idx Reduction + MaskRCNN',
            ...
            'parameters':
                {   
                    ...
                    'pansharpening': {'values': ['Simple Mean']},
                    'dimensionality_reduction': {'values': ['Vegetation Indexes']},
                    'scaling': {'values' : [True, False]},
                    'drop_little_objets': {'values': [True, False]},
                    'drop_objects_area': {'values': [i**2 for i in range(3,10)]}, 

                    # Model config
                    #'backbone': {'values': ['ResNet-50-FPN', 'ResNet-50-FPN V2', 'MobileNet V2']},
                    'backbone': {'values': ['resnet50-fpn','resnet50-fpn-v2'] + ['efficientnet_b{}'.format(i) for i in [0,2]] + ['mobilenetv2_100','resnext101_32x8d','resnet101']},
